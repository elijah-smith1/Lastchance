...
struct CreateAccountView: View {
    ...
    var body: some View {
        NavigationStack { // Wrap VStack with NavigationView
            VStack {
                ...
                NavigationLink(destination: SelectInterests(uid: self.uid)
                    .environmentObject(self.userData), isActive: $showSelectInterestPage) {
                        EmptyView()
                    }
            }
    ...
    private func createAccount() {
        print("UID in CreateAccount: \(uid)")
        guard !uid.isEmpty else {
            print("Error: UID is empty in CreateAccount.")
            return
        }
        ...
        let usernameRef = db.collection("usernames").document(userData.username)
        ...
        usernameRef.getDocument { (document, error) in
            if let error = error {
                ...
                if let document = document, document.exists {
                    self.alertMessage = "Username already exists"
                    self.showingAlert = true
                } else {
                    // Username is unique, proceed to upload image and set user data
                            ...
                            storageRef.downloadURL { (url, error) in
                                if let error = error {
                                    ...
                                    let userData = [
                                        ...
                                        "profilePictureURL": downloadURL, // Assuming this has the appropriate URL string
                                        "interests": self.userData.interests, // This is an array of strings
                                        "followerCount": self.userData.followerCount,
                                        "favorites": self.userData.favorites,
                                        "followingCount": self.userData.followingCount,
                                        "postCount": self.userData.postCount,
                                        "isVendor": self.userData.isVendor // Firebase can handle booleans, so no need for conversion
                                    ] as [String : Any] // Firestore dictionary type
                                    ...
                                    db.collection("users").document(uid).setData(userData) { error in
                                        if let error = error {
                                            ...
                                            usernameRef.setData(["uid": uid]) { error in
                                                if let error = error {
                                                    print("Error setting document: \(error)")
                                                } else {
                                                    DispatchQueue.main.async {
                                                        self.showSelectInterestPage = true
                                                    }
                                                }
                                            }
                                        }
...
struct CreateAccountView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        if let currentUser = Auth.auth().currentUser {
            ...
            // Now you can use this uid for your operations.
        
            return CreateAccountView(uid: uid)
                .environmentObject(UserData()) // Add the environment object here
        }

        // Fallback preview for a non-logged-in user
        return CreateAccountView(uid: "MjtdfqAyCuhOE8S8MbigBT9okXN2")
            .environmentObject(UserData()) // Add the environment object here
    }
}
